...
const AdminJobs: React.FC<AdminJobsProps> = ({ companyFilter }) => {
  const [activeView, setActiveView] = useState<'calendar' | 'gps-routes'>('calendar');
  ...
  return (
    <div className="space-y-4 px-1.5 md:px-2.5 text-left pb-6 w-full max-w-full">
      ...
      {/* 2. Top View Switcher (Calendar Schedule vs Google Maps Daily Route & KM Tracker) */}
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/80 dark:border-slate-800 pb-3">
        <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl border border-slate-200/60 dark:border-slate-800">
          <button
            onClick={() => setActiveView('calendar')}
            className={`flex items-center space-x-1.5 px-4 py-2 text-xs font-extrabold rounded-lg transition-all cursor-pointer ${
              activeView === 'calendar'
          ...
          >
            ...
            <span>📅 Job Scheduling Calendar</span>
          </button>
          <button
            onClick={() => setActiveView('gps-routes')}
            className={`flex items-center space-x-1.5 px-4 py-2 text-xs font-extrabold rounded-lg transition-all cursor-pointer ${
              activeView === 'gps-routes'
                ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20 font-black'
                : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
            }`}
          >
            <Navigation className="h-3.5 w-3.5" />
            <span>🚗 Worker Daily GPS Routes & KM Tracker</span>
          </button>
        </div>

        {activeView === 'calendar' && (
          <div className="flex space-x-2 text-[11px] font-bold shrink-0">
      ...
      {activeView === 'gps-routes' ? (
        ...
        /* 3. Main Split Panel Container: Grid + Drawer */
