...
const LiveActiveJobBanner: React.FC<{ job: any }> = ({ job }) => {
  ...
  return (
    <div className="rounded-2xl border border-emerald-500/30 bg-emerald-50/10 dark:bg-emerald-950/5 backdrop-blur-md p-4 space-y-3 shadow-md relative overflow-hidden text-slate-800 dark:text-white text-xs">
      ...
      <div className="grid grid-cols-1 gap-2 pt-2.5 border-t border-slate-100 dark:border-slate-800/80 text-left text-[11px] text-slate-650 dark:text-slate-350">
        ...
        <div>
          <span className="text-slate-450 block text-[9px]">Client: <strong className="text-slate-700 dark:text-slate-205">{job.clientName || 'N/A'}</strong></span>
          <span className="text-slate-450 block text-[9px] truncate mt-0.5">Address: <strong className="text-slate-700 dark:text-slate-205">📍 {job.address || 'N/A'}</strong></span>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="relative min-w-[200px] w-full md:w-auto text-left">
            ...
            <input
              ...
              onChange={(e) => {
                setSearchWorkerName(e.target.value);
            ...
            {showWorkersDropdown && (
              <div className="absolute z-[999] top-full left-0 right-0 max-h-60 overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg mt-1 text-left">
                {workers
                  .filter((w: any) => w.name.toLowerCase().includes(searchWorkerName.toLowerCase()))
                  .map((w: any) => (
                    <div 
                      key={w._id} 
                      onClick={() => {
                        setSearchWorkerName(w.name);
                ...
                {workers.filter((w: any) => w.name.toLowerCase().includes(searchWorkerName.toLowerCase())).length === 0 && (
